...
static void simpleexp(LexState *ls) {
    ...
    switch (ls->t.token) {
        ...
        case '`':
        case TK_QUOTE: { /* quote expression */
            doquote(ls, ls->t.token == '`');
            return;
        }
        case TK_ESCAPE: {
        ...
        case TK_TERRA: {
            ...
            int id = store_value(ls);
            OutputBuffer_printf(&ls->output_buffer,
                                "terra.anonfunction(_G.terra._trees[%d],", id);
            print_captured_locals(ls, &tc);
            OutputBuffer_printf(&ls->output_buffer, ")");
            luaX_patchend(ls, &begin);
            leaveterra(ls);
        ...
        case TK_STRUCT: {
            ...
            luaX_next(ls);  // skip over struct, struct constructor expects it to be
                            // parsed already
            ...
            luaX_patchbegin(ls, &begin);
            OutputBuffer_printf(&ls->output_buffer,
                                "terra.anonstruct(_G.terra._trees[%d],", id);
            print_captured_locals(ls, &tc);
            OutputBuffer_printf(&ls->output_buffer, ")");
            luaX_patchend(ls, &begin);
        ...
        case TK_RSHIFT:
            if (!ls->in_terra)
                luaX_syntaxerror(
                        ...
                        luaS_cstringf(
                                ls->LP,
                                "@, <<, and >> operators not supported in Lua code."));
            break;
        case '#':
            if (ls->in_terra)
                luaX_syntaxerror(
                        ls,
                        luaS_cstringf(ls->LP, "# operator not supported in Terra code."));
            break;
        default:
            break;
    }
...
static BinOpr subexpr(LexState *ls, int limit) {
    ...
    if (op == OPR_FUNC_PTR) {
        ...
        while (isspace(*lhs_string)) lhs_string++;
    }
    while (op != OPR_NOBINOPR && priority[op].left > limit) {
        ...
        if (lhs_string) {  // desugar a -> b to terra.types.functype(a,b)
            ...
            luaX_patchbegin(ls, &begintoken);
            OutputBuffer_printf(&ls->output_buffer, "terra.types.funcpointer(%s,%s)",
...
static void expr(LexState *ls) { RETURNS_1(subexpr(ls, 0)); }
...
static void embeddedcode(LexState *ls, int isterra, int isexp) {
    ...
    data.step =
            (isexp || isterra)
                    ? 0
                    : 1;  // is the string we captured an expression? it is if we captured
                          // a quote (terra.definequote(...)) or a lua expression
    ...
    if (lua_load(ls->L, expr_reader, &data, ss.str().c_str()) != 0) {
        ...
        // we need to find the line number in the error string, add it to where we began
        // this line, and then report the error with the correct line number
        ...
        luaX_reporterror(ls, luaS_cstringf(ls->LP, "%s:%d: %s\n", getstr(ls->source),
                                           begintoken.seminfo.linebegin + lineoffset - 1,
                                           aftererror + 1));
    }
...
static void luaexpr(LexState *ls) { embeddedcode(ls, 0, 1); }

static void terratype(LexState *ls) { luaexpr(ls); }
...
static void fornum(LexState *ls, TString *varname, Position *p) {
    ...
    BlockCnt bl;
    if (varname) definevariable(ls, varname);
...
static void forlist(LexState *ls, TString *indexname, Position *p) {
    ...
    BlockCnt bl;
    if (indexname) definevariable(ls, indexname);

    while (testnext(ls, ',')) {
        TString *name;
        if (vardecl(ls, 0, &name)) definevariable(ls, name);
        add_entry(ls, vars);
    }
    checknext(ls, TK_IN);
    int line = ls->linenumber;
...
static void forstat(LexState *ls, Position *p) {
    ...
    switch (ls->t.token) {
        case '=':
            RETURNS_0(fornum(ls, varname, p));
            break;
        case ',':
        case TK_IN:
            RETURNS_0(forlist(ls, varname, p));
            break;
        default:
...
void print_name_list(LexState *ls, std::vector<Name> *definednames) {
    for (size_t i = 0; i < definednames->size(); i++) {
        ...
        if (i + 1 < definednames->size()) OutputBuffer_putc(&ls->output_buffer, ',');
    }
}
...
static void localstat(LexState *ls) {
    ...
    do {
        TString *vname;
        if (vardecl(ls, 0, &vname)) declarednames.push_back(vname);
        add_entry(ls, vars);
    } while (testnext(ls, ','));
    if (testnext(ls, '=')) {
        push_boolean(ls, true);
...
bool startsTerraStat(LexState *ls) {
    if (ls->t.token == TK_TERRA || ls->t.token == TK_STRUCT) return true;
...
static void terrastats(LexState *ls, bool emittedlocal) {
    ...
    for (int idx = 0; true; idx++) {
        ...
        if (islocal)
            ...
            refvariable(ls, name->data[0]);  // non-local names are references because we
                                             // look up old value
        TDefn tdefn;
        tdefn.treeid = -1;
        tdefn.islocal = islocal;
        switch (token) {
            case TK_TERRA: {
    ...
    if (!emittedlocal && nlocals > 0) OutputBuffer_printf(&ls->output_buffer, "local ");
    for (size_t i = 0, emitted = 0; i < names.size(); i++) {
        ...
        if (defs[i].islocal) {
            ...
            tc.capturedlocals.insert(firstname);  // capture these locals so that the
                                                  // constructor doesn't see old values
            if (++emitted < nlocals) OutputBuffer_putc(&ls->output_buffer, ',');
        }
    }
    if (nlocals > 0) OutputBuffer_putc(&ls->output_buffer, ';');
    ...
    if (hasreturns) OutputBuffer_printf(&ls->output_buffer, " = ");
    ...
    for (size_t i = 0; i < defs.size(); i++) {
        ...
        OutputBuffer_printf(&ls->output_buffer, ", \"");
        Name_print(name, ls);
        OutputBuffer_printf(&ls->output_buffer, "\", _G.terra._trees[%d]",
                            defs[i].treeid);
    }
...
static void exprstat(LexState *ls) {
    ...
    if (ls->t.token != '=' && ls->t.token != ',') { /* stat -> func */
        ...
        add_entry(ls, tbl);
        RETURNS_0(assignment(ls, 1, tbl));  // assignment will replace this table with the
                                            // actual assignment node
    }
}
...
static int le_next(lua_State *L) {
    LexState *ls = const_cast<LexState *>(
            (const LexState *)lua_topointer(L, lua_upvalueindex(1)));
    try {
        luaX_next(ls);
    } catch (...) {
        le_handleerror(ls);
...
static int le_embeddedcode(lua_State *L) {
    ...
    lua_remove(ls->L,
               -2); /* remove original object, we just want to return the function */
    return 1;
}
...
static void languageextension(LexState *ls, int isstatement, int islocal) {
    ...
    enterterra(ls, &tc);  // specifically to capture local variables, not for other
                          // reasons
...
int luaY_parser(terra_State *T, ZIO *z, const char *name, int firstchar) {
    ...
    TString *tname = (name[0] == '@') ? luaS_new(T, name + 1)
                                      : luaS_stringf(T, "[string \"%s\"]", name);
    lexstate.buff = buff;
    ...
    try {
        ...
        if (lua_gettop(L) > lexstate.stacktop + 1) {
            lua_replace(L, lexstate.stacktop +
                                   1);  // put the error message at the new top of stack
            lua_settop(
                    L,
                    lexstate.stacktop + 1);  // reset the stack to just 1 above where
                                             // it orignally (holding the error message)
        }
    ...
    int err = luaL_loadbuffer(L, lexstate.output_buffer.data, lexstate.output_buffer.N,
                              name);
    cleanup(&lexstate);
