...
| `A.methods.__move(from : &A, to : &A)`         | Always auto-generated to handle move semantics for resource transfer.        |
| `__move__`                                     | Prefer `__move` over `__copy` for efficient resource transfer.               |
| `__handle__`                                   | Get a handle to a managed object without transferring ownership.             |
...
In nested scopes with return statements all resources need to be cleaned up before the return. Consider the following contrived example. Note that deferred destructor calls are scheduled in reverse order of object creation. When the stack is unwound in the return of the `if` block, first `a` is destroyed, then `b` and then `c`.
```
terra f(a : A)
    var k = 0
    while true do
        ...
        if k > 2 then
            var c : A
            var d : A
            defer A.__dtor(&a)  --generated by the compiler
            defer A.__dtor(&b)  --generated by the compiler
            defer A.__dtor(&c)  --generated by the compiler
            defer A.__dtor(&d)  --generated by the compiler
            return k
        end
        var e : A
        var f : A
        defer A.__dtor(&b)  --generated by the compiler
        defer A.__dtor(&e)  --generated by the compiler
        defer A.__dtor(&f)  --generated by the compiler
    end
    defer A.__dtor(&a)  --generated by the compiler
    return 0
end
...
## ToDo:
1. There is a bug in destructor calls in return statements and in break statements. The order of destruction should be reversed.
2. Implement `__copy` and `__move` for arrays.
3. Check that `b[i] = a(j)` works for managed arrays.
3. Maybe rewrite `__move` using `memcopy` for efficiency.
4. Track (field) initialization and add compiler-checks
5. Add field-based initializers directly in struct definition.
